...
deploy_lambda() {
  ...
  if aws lambda get-function --function-name $FN --region $REGION 2>/dev/null; then
    ...
    aws lambda create-function --function-name $FN \
      ...
      --layers $LAYER_ARN --region $REGION > /dev/null
    ok "$FN creado"
  fi
}
deploy_lambda "itx-router"       "itx-lambda-router-role"    "handler.lambda_handler" 60  256
deploy_lambda "itx-transform"    "itx-lambda-transform-role" "handler.lambda_handler" 900 1024
deploy_lambda "itx-extract"      "itx-lambda-extract-role"   "handler.lambda_handler" 900 1024
deploy_lambda "itx-clean"        "itx-lambda-clean-role"     "handler.lambda_handler" 900 1024
...
aws lambda update-function-configuration --function-name itx-transform --region $REGION \
  --environment "Variables={S3_BUCKET_LANDING=${S3_LANDING},S3_BUCKET_STAGING=${S3_STAGING},CHUNK_SIZE_MB=64,FLUSH_BATCH_SIZE=500000}" > /dev/null
...
aws glue create-job --name itx-calculate --role $GCALC_ROLE \
  ...
  --glue-version "3.0" --number-of-workers 2 --worker-type Standard \
...
echo " S3 Buckets     : 5"
echo " IAM Roles      : 11"
echo " Lambda Layer   : itx-pandas-pyarrow v1"
echo " DynamoDB       : 4 tablas"
echo " Lambdas        : 5 (itx-store pendiente)"
echo " S3 Event       : itx-landing -> itx-router"
echo " Step Functions : itx-main-orchestrator"
echo " Glue Jobs      : itx-calculate, itx-interchange"
...
echo " - Cargar datos en DynamoDB (file-pattern, visa-fields, client)"
